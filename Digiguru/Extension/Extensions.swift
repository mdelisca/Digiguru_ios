...
extension String {
    ...
    func getTimeFromString() -> String{
        let inFormatter = DateFormatter()
        inFormatter.setLocalizedDateFormatFromTemplate("yyyy-MM-dd HH:mm:ss")
        inFormatter.locale = Locale(identifier: "fr-CA")
        inFormatter.timeZone = TimeZone(abbreviation: "EDT")
    ...
    func getDateTimeFromString() -> String{
        ...
        let inFormatter1 = DateFormatter()
        inFormatter1.dateFormat = "yyyy-MM-dd HH:mm:ss"
    ...
    func getDateTimeFromStringToDate(_ date:String,_ time:String) -> Date{
        let inDFormatter = DateFormatter()
        inDFormatter.dateFormat = "yyyy-MM-dd"
        ...
        if let datestr = inDFormatter.date(from: date),let timestr = inTFormatter.date(from: time){
            if let combine = combineDateWithTime(date: datestr, time: timestr){
                return combine
            }
            else{
                return Date()
    ...
    func convertCurrencyStyle(places:Int)->String{
        ...
        if let val = formatter.number(from: self) {
            if let vals = formatter.string(from: val){
                return vals
            }
            else{
                return "0.0"
