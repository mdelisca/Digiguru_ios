...
class CommonHelper
{
    ...
    class func saveCachedUserData(_ userData:UserModel){
        ...
        do {
            ...
            try userDefaults.setObject(userData, forKey: Constant.login_user)
...
//    }
    class func getCachedUserData() -> UserModel? {
        let userDefaults = UserDefaults.standard
        do {
            let user = try userDefaults.getObject(forKey: Constant.login_user, castTo: UserModel.self)
            print(user.userType)
            return user
        } catch {
            print(error.localizedDescription)
...
//    }
    ...
    func changeNAV_VC(identifier:String,vc:UIViewController) -> UIViewController {
